...
def read_double(file): # wk
    """Read 8-byte as double."""
    binary = file.read(8)
    return struct.unpack('<d', binary)
...
# def bytes_to_bits_binary(file, length, byteorder='little'):
#     byte_data = read_byte_by(file, length)
#     bits_data = bin(int.from_bytes(byte_data, byteorder=byteorder))[2:]
#     return bits_data

from bitarray import bitarray
def read_bits(file, length) -> list[int]:
    bits_data = bitarray(endian='little')
    bits_data.frombytes(read_byte_by(file, length))
    return bits_data.tolist()
...
def read_byte_by(file, byte_cnt):
    """Read 16 bytes."""
    return file.read(byte_cnt)

def read_bool(file):
    """Read bool 1 bytes."""
    return read_bool_by(file, 1)

def read_bool_by(file, length):
    """Read bool length bytes."""
    return bool(int.from_bytes(file.read(length), 'little'))
